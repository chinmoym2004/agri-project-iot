...
class DataBase_Access_Model():
    ...
    def query_table(self, key):
        return _aws_dynamodb.query_items(self._tablename, Key('farm_id').eq(key))

    def scan_table(self, filter_experssion):
        response = _aws_dynamodb.scan_table(self._tablename, filter_experssion)
        return response
    
    def update_value(self, part_key, sort_key, expressionAttrVal):
        return _aws_dynamodb.update_item(self._tablename,{'device_id': part_key, 'farm_id': sort_key},'set device_state = :r',{':r':expressionAttrVal,})
    
    def scan_all_items(self):
        return _aws_dynamodb.scan_entire_table(self._tablename)
    ...
    def get_data(self,sort_key, part_key):
        return(_aws_dynamodb.get_item(self._tablename,{'device_id':sort_key,'farm_id':part_key}))
